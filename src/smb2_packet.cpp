    ...
    : m_data(data), m_length(length)
{
    ...
    if (le16toh(hdr->structure_size) != 64) {
...
nlohmann::json SMBv2Packet::toJson() const {
    ...
    j["command"] = smb2_command_to_string(le16toh(hdr->command));
    j["command_code"] = le16toh(hdr->command);
    j["status"] = ntstatus_to_string(le32toh(hdr->status));
    j["message_id"] = le64toh(hdr->message_id);
    j["session_id"] = le64toh(hdr->session_id);
    j["tree_id"] = le32toh(hdr->tree_id);
    j["credit_request"] = le16toh(hdr->credit_request);
    j["credit_charge"] = le16toh(hdr->credit_charge);
    j["flags"] = le32toh(hdr->flags);
    j["has_next"] = hasNextCommand();
    j["data_size"] = commandParamsSize();

    // Command-specific params
    nlohmann::json params;
    uint16_t cmd = le16toh(hdr->command);
    ...
    if (cmd == SMB2_NEGOTIATE && psize >= sizeof(Smb2NegotiateResponse)) {
        ...
        params["dialect_revision"] = smb2_dialect_to_string(le16toh(nr->dialect_revision));
        params["capabilities"] = le32toh(nr->capabilities);
        params["max_read_size"] = le32toh(nr->max_read_size);
        params["max_write_size"] = le32toh(nr->max_write_size);
        char guid[33];
        ...
        if (le16toh(cr->structure_size) == 89) {
            char fid[33];
            for (int i = 0; i < 16; i++) snprintf(fid + i*2, 3, "%02X", cr->file_id[i]);
            params["file_id"] = fid;
            params["end_of_file"] = le64toh(cr->end_of_file);
            params["allocation_size"] = le64toh(cr->allocation_size);
            params["oplock_level"] = cr->oplock_level;
            params["create_action"] = le32toh(cr->create_action);
        }
    }
    else if (cmd == SMB2_READ && psize >= sizeof(Smb2ReadResponse)) {
        auto* rr = static_cast<const Smb2ReadResponse*>(commandParams());
        if (le16toh(rr->structure_size) == 17) {
            params["data_length"] = le32toh(rr->data_length);
            params["data_offset"] = rr->data_offset;
            params["data_remaining"] = le32toh(rr->data_remaining);
        }
    }
    else if (cmd == SMB2_WRITE && psize >= sizeof(Smb2WriteRequest)) {
        auto* wr = static_cast<const Smb2WriteRequest*>(commandParams());
        params["length"] = le32toh(wr->length);
        params["offset"] = le64toh(wr->offset);
        char fid[33];
