...
nlohmann::json SMBv1Packet::toJson() const {
    ...
    j["flags"] = hdr->flags;
    j["flags2"] = static_cast<int>(smb_le16toh(hdr->flags2));
    j["pid"] = (static_cast<uint32_t>(smb_le16toh(hdr->pid_high)) << 16) | smb_le16toh(hdr->pid_low);
    j["tid"] = smb_le16toh(hdr->tid);
    j["uid"] = smb_le16toh(hdr->uid);
    j["mid"] = smb_le16toh(hdr->mid);
    ...
    switch (cmd) {
        case SMB_COM_NEGOTIATE:
            if (m_param_offset + sizeof(Smb1NegotiateResponse) <= m_length) {
                ...
                params["dialect_index"] = smb_le16toh(nr->dialect_index);
                params["security_mode"] = static_cast<int>(nr->security_mode);
                params["max_buffer_size"] = smb_le32toh(nr->max_buffer_size);
                params["capabilities"] = smb_le32toh(nr->capabilities);
                params["max_mpx_count"] = smb_le16toh(nr->max_mpx_count);
                params["max_vc_count"] = smb_le16toh(nr->max_vc_count);
            }
            break;
        ...
        case SMB_COM_SESSION_SETUP_ANDX:
            if (m_param_offset + sizeof(Smb1SessionSetupAndXRequest) <= m_length) {
                ...
                params["security_blob_length"] = smb_le16toh(req->security_blob_length);
                params["capabilities"] = smb_le32toh(req->capabilities);
            }
            break;
        case SMB_COM_TREE_CONNECT_ANDX:
            if (m_param_offset + sizeof(Smb1TreeConnectAndXResponse) <= m_length) {
                ...
                params["optional_support"] = smb_le16toh(resp->optional_support);
            }
            break;
    }
